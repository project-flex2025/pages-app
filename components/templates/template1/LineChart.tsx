...
// Enhanced format function that handles both dates and text
  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-2 rounded shadow text-sm bg-white">
        <p className="font-semibold">{formatXValue(label || "")}</p>
...
const isDateData = (data: ChartDataItem[], xKey: string) => {
  ...
  try {
    const date = new Date(data[0][xKey]);
...
const Template1LineChart: React.FC<TemplateLineChartProps> = ({
  widgetData,
  formattedData,
  loading,
  error,
  ...
  if (!formattedData.length)
    return <div className="alert alert-info mx-3 my-4">No data available</div>;
...
export default Template1LineChart;
